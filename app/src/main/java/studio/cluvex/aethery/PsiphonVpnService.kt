...
class PsiphonVpnService : VpnService(), PsiphonTunnel.HostService {
    ...
    private fun startTunnelBg() {
        try {
            ...
            tunnel?.setVpnMode(false) // Proxy mode — AetherVpnService+TUN handles full tunnel later
    ...
    private fun fetchPublicIpBg() {
        try {
            // Route through Psiphon's local SOCKS proxy to see exit IP + flag
    ...
    private fun socksHttpGet(url: String, proxy: Proxy): String? {
        try {
            ...
            Log.v(TAG, "socksHttpGet failed: $url $e")
            return null
        }
    }
